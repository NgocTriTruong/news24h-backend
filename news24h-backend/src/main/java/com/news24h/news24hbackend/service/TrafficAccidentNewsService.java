...
public class TrafficAccidentNewsService {
    ...
    public List<NewsArticle> crawlTrafficAccidentNews() {
        List<NewsArticle> articles = new ArrayList<>();

        try {
            String url = "https://www.24h.com.vn/tai-nan-giao-thong-c408.html";
                ...
                try {
                    ...
                    if (!title.isEmpty() && title.length() > 10 &&
                        ...
                        if (newsRepository.findBySourceUrl(href).isEmpty()) {
                            ...
                            try {
                                ...
                                if (parent != null) {
                                    ...
                                    if (imgInParent != null) {
                                        ...
                                        if (imgInParent.hasAttr("srcset") && !imgInParent.attr("srcset").isEmpty()) {
                                            String srcset = imgInParent.attr("srcset");
                                            // chọn phần tử đầu tiên trong srcset
                                            String first = srcset.split(",")[0].trim().split("\\s+")[0];
                                            thumbnail = imgInParent.baseUri().isEmpty() ? first : Jsoup.parse("<a href='" + first +"'>").selectFirst("a").attr("abs:href");
                                        }
                                        if ((thumbnail == null || thumbnail.isEmpty()) && imgInParent.hasAttr("data-src")) {
                                            thumbnail = imgInParent.attr("abs:data-src");
                                        }
                                        if ((thumbnail == null || thumbnail.isEmpty()) && imgInParent.hasAttr("data-original")) {
                                            thumbnail = imgInParent.attr("abs:data-original");
                                        }
                                        if ((thumbnail == null || thumbnail.isEmpty()) && imgInParent.hasAttr("src")) {
                                            thumbnail = imgInParent.attr("abs:src");
                                        }
                                        // Loại bỏ placeholder data URI/1x1 gif
                                        if (thumbnail != null) {
                                            String t = thumbnail.toLowerCase();
                                            if (t.startsWith("data:image") || t.contains("r0lgodlhaqab")) {
                                                thumbnail = null;
                                            }
                                        }
                                    }
                            ...
                            article.setDescription("Tin tức Tai nạn giao thông");
                            article.setThumbnail(thumbnail);
                            article.setCategory("tai-nan-giao-thong");
            ...
            System.err.println("Lỗi crawl tai-nan-giao-thong: " + e.getMessage());
        }
    ...
    @Scheduled(fixedRate = 43200000, initialDelay = 85000)
    public void scheduledCrawl() {
        System.out.println("Bắt đầu crawl tin Tai nạn giao thông tự động...");
        crawlTrafficAccidentNews();
    }
}
